...
const EditResume = () => {
  ...
  const calculateCompletion = () => {
    ...
    completedFields += resumeData.interests.filter((i) => i.trim() !== "")
      .length;
...
// UPDATE THE SECTION
  ...
  // ADDING NEW ITEM IR INFORMATION
  ...
  // Replace your existing uploadResumeImages with this debug version
const uploadResumeImages = async () => {
  try {
    setIsLoading(true);

    const thumbnailWrapper = thumbnailRef.current;
    if (!thumbnailWrapper) {
      throw new Error("Thumbnail element not found (thumbnailRef is null)");
    }

    // pick a reasonable capture target inside the wrapper (a4-wrapper or first child)
    const candidate =
      thumbnailWrapper.querySelector?.(".a4-wrapper, .resume-section, .hiddenThumbnail") ||
      thumbnailWrapper.firstElementChild ||
      thumbnailWrapper;

    if (!candidate || !(candidate instanceof Element)) {
      throw new Error("Invalid element provided as first argument (no valid child to capture)");
    }

    // capture — your helper returns a dataURL
    const thumbnailDataUrl = await captureElementAsImage(candidate);
    if (!thumbnailDataUrl) throw new Error("captureElementAsImage returned falsy value");

    // convert to File
    const thumbnailFile = dataURLtoFile(thumbnailDataUrl, `thumbnail-${resumeId || "temp"}.png`);
    console.log("thumbnailFile:", thumbnailFile, "size:", thumbnailFile.size, "type:", thumbnailFile.type);

    // build FormData and LOG entries (so you can inspect in DevTools console)
    const formData = new FormData();
    // --- CRITICAL: confirm the expected field name on your backend (try 'thumbnail', 'image', or 'file') ---
    formData.append("thumbnail", thumbnailFile);
    // if backend expects another field name, change the string above accordingly

    // Log FormData entries (can't print whole formData directly)
    for (let pair of formData.entries()) {
      console.log("FormData entry:", pair[0], pair[1]);
    }

    // Send request — DO NOT set Content-Type manually; axios will set boundary for multipart/form-data
    ...
    console.log("Upload response:", uploadResponse && uploadResponse.data);
    const { thumbnailLink } = uploadResponse.data || {};
    await updateResumeDetails(thumbnailLink);
    toast.success("Resume Updated Successfully");
    navigate("/dashboard");
  } catch (error) {
    // Print as much info as possible so you can see server content
    console.error("Error Uploading Images:", error);
    if (error?.response) {
      console.error("Server response status:", error.response.status);
      console.error("Server response data:", error.response.data);
      toast.error("Failed to upload images: " + (error.response.data?.message || error.response.data || error.message));
    } else {
  ...
  // THE FUNCTION WILL HELP IN UPDATING THE RESUMES. AND PUT WILL HELP IN UPDATION ON BACKEND.
  ...
  // SO ALL THE FUNCTION WILL BE USED FOR THE EDIT RESUMES.
  ...
  return (
    ...
    {/* Preview modal: fixed, single Modal block with content + action button */}
    <Modal
    ...
    {/* Hidden thumbnail (kept for your image upload flow) */}
    {/* Hidden thumbnail (render off-screen so html2canvas can capture it) */}
<div
  ref={thumbnailRef}
  style={{
    position: "absolute",
